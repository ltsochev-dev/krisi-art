 ...
 *   across page loads — which is what lets a visitor's browser cache hold an
 *   album it has already looked at. See `GALLERY_URL_WINDOW_SECONDS`.
 *
 * **Two URLs per photograph**, because there are two objects: the original at
 * whatever size it came off the camera, and the 512px preview written beside it
 * when the file was registered. The grid draws the preview and the viewer draws
 * the original. Neither passes through this server — a tile is a presigned `GET`
 * of a twenty-kilobyte object, which is the whole point of storing it; see
 * `@/lib/commissions/thumbnails` for what asking the image optimiser to make
 * that copy per request did to a 220-photo album.
 *
 * `thumbUrl` is `null` for a row that has no preview — one uploaded before
 * previews existed, or one whose resize failed — and the grid falls back to
 * `next/image` on the original for it. That fallback is the arrangement this
 * replaced, so it works; it is just the slow path, and it disappears for an
 * album the moment the artist presses Generate previews.
 *
 ...
 * so it is not recorded as a download. (Out of the grid they save the stored
 * preview rather than the photograph, which is what the page's footnote warns
 * about.) The viewer's own download button goes through the endpoint and is
 * recorded. That is the honest trade for a page whose whole job is showing
 * pictures, and it is why the view counter, not the download counter, is the
 * meaningful number on a gallery.
 */
...
/** A file the grid can draw, with the URLs to draw it from. */
export type CommissionGalleryImage = {
  /** The stored preview, or `null` when this row has none yet. */
...
export const buildCommissionGallery = async ({
  ...
  for (const row of rows) {
    ...
    /**
     * Shared by both signatures below.
     *
     * `bucket` is explicit and never the helper's default — that one is the
     * media bucket, which is public through a CDN, and a commission key must
     * not be signed against it. `expiresIn` is two windows because the
     * signature is dated to the *start* of the current one: a URL minted in its
     * closing seconds would otherwise arrive at the browser already spent.
     */
    const common = { bucket, expiresIn: windowSeconds * 2, signingDate }
    ...
    signing.push(
      Promise.all([
        ...
        /**
         * The preview, which carries neither of those two response overrides.
         * It is WebP whatever the original was, so a `Content-Disposition`
         * naming it `sunset.jpg` would be a lie told to the one visitor who
         * right-clicks a tile, and the object's own stored `Content-Type` is
         * already correct.
         */
        row.thumbKey
          ? getPresignedDownloadUrl({ ...common, key: row.thumbKey })
          : Promise.resolve(null),
      ]).then(([url, thumbUrl]) => ({ ...file, thumbUrl, url })),
    )
  }
