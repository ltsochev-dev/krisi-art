 ...
 *
 * All six are collection endpoints, so they are served by the existing Payload
 * catch-all at `src/app/(payload)/api/[...slug]/route.ts` and need no new Next
 * route files. Two of them are public; four are for editors.
 *
 ...
 *
 * The public two are keyed on the **UUID** and the editor four on the numeric
