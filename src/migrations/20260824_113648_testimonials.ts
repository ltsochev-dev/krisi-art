...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`testimonials_socials\` (
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`testimonials_socials_order_idx\` ON \`testimonials_socials\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`testimonials_socials_parent_id_idx\` ON \`testimonials_socials\` (\`_parent_id\`);`,
  )
  ...
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`testimonials_id\` integer REFERENCES testimonials(id);`,
  )
  await db.run(
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`testimonials_socials\`;`)
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "albums_id", "artworks_id", "tags_id", "media_id", "contact_submissions_id", "users_id") SELECT "id", "order", "parent_id", "path", "albums_id", "artworks_id", "tags_id", "media_id", "contact_submissions_id", "users_id" FROM \`payload_locked_documents_rels\`;`,
  )
